...
.package-category__heading{margin-bottom:1.25rem}
...
if (packageCategory && homeGrid) {
  packageCategory.id = "home-packages";
  const plans = [
  ...
  [...homeGrid.querySelectorAll(".price-card")].slice(0, 7).forEach((card, i) => {
    const [speed, price, description] = plans[i];
    const title = card.querySelector("h3");
    const amount = card.querySelector("strong");
    const desc = card.querySelector("p");
    if (title) title.textContent = speed;
    if (amount) amount.innerHTML = `${price} <span>/ month</span>`;
    if (desc) desc.textContent = description;
    if (i === 3) card.classList.add("price-card--featured");
    else card.classList.remove("price-card--featured");
  });
  const existing = homeGrid.querySelectorAll(".price-card");
  [...existing].slice(7).forEach(card => card.remove());
}
...
if (!document.querySelector("#student-package") && packageCategory) {
  ...
  student.innerHTML = `
    <span class="eyebrow">Student Connectivity</span>
    <h3>Student Package</h3>
    <p>Affordable unlimited internet for students to study, attend online classes, research, submit assignments and stay connected.</p>
    <div class="student-package-card">
      <div><span class="eyebrow">Student Home Plan</span><strong>10 Mbps</strong><strong>KSh 600 <small>/ month</small></strong><ul><li>Unlimited home internet</li><li>Designed for learning and study</li><li>Coverage confirmation applies</li></ul><a class="btn btn--solid" href="#contact">Request Student Package</a></div>
      <div><span class="eyebrow">Built For Learning</span><h3>Stay connected to opportunity.</h3><p>Reliable access for classes, research, assignments, applications, communication and online resources.</p></div>
    </div>`;
...
if (!document.querySelector("#community-package") && packageCategory) {
  ...
  community.innerHTML = `
    <span class="eyebrow">Serving The Community</span>
    <h3>Community Connect — Accessible Connectivity</h3>
    <p>NIS supports customers with disabilities with affordable home internet for communication, study, work and daily access.</p>
    <div class="community-access-card">
      <img src="assets/images/community-accessible-new.jpg" alt="Customer using a laptop while seated in a wheelchair">
      <div><span class="eyebrow">Customers With Disabilities</span><strong>10 Mbps — KSh 600 / month</strong><p>Unlimited home internet for eligible customers with disabilities. Coverage confirmation applies before installation.</p><a class="btn btn--solid" href="#contact">Request Community Package</a></div>
    </div>`;
